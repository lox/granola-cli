...
type AuthCmd struct {
	Login   AuthLoginCmd   `cmd:"" help:"Authenticate with Granola via OAuth"`
...
func (c *AuthRefreshCmd) Run(ctx *Context) error {
	...
	if err != nil {
		if err == mcp.ErrNoToken {
			output.PrintWarning("Not authenticated. Run 'granola auth login' first.")
			return err
		}
		output.PrintError(err)
		return err
	...
	if token.RefreshToken == "" {
		output.PrintWarning("No refresh token available. Run 'granola auth login' to re-authenticate.")
...
func (c *AuthStatusCmd) Run(ctx *Context) error {
	...
	if err != nil {
		if err == mcp.ErrNoToken {
			fmt.Println("Not authenticated. Run 'granola auth login' to authenticate.")
			return nil
		}
		output.PrintError(err)
		return err
